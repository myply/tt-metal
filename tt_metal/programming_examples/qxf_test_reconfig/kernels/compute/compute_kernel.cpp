...
void MAIN {
    ...
    for (uint32_t i = start_id; i < end_id; i += block_size) {
        ...
        for (uint32_t tile_index = 0; tile_index < block_size; ++tile_index) {
            tile_regs_acquire();
            // reconfig_data_format_srca<true>(tt::CBIndex::c_0, tt::CBIndex::c_0); // TRISC kernel duration around 182xx ~ 184xx 
            // reconfig_data_format_srca<true>(tt::CBIndex::c_0); // TRISC kernel duration around 176xx ~ 178xx
            asm("nop");
            asm("nop");
