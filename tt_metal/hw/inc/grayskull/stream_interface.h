...
inline __attribute__((always_inline)) uint32_t stream_phase_advance_wait(uint32_t stream_id) {
    ...
    uint32_t num_tiles_pending =
        NOC_STREAM_READ_REG(stream_id, STREAM_DEBUG_STATUS_REG_INDEX + 8) >> MEM_WORD_ADDR_WIDTH;
...
inline __attribute__((always_inline)) int stream_get_curr_phase_num_msgs(uint32_t stream_id) {
    return NOC_STREAM_READ_REG_FIELD(
        stream_id,
        STREAM_PHASE_AUTO_CFG_HEADER_REG_INDEX,
        PHASE_NUM_INCR);  // is actually CURR_PHASE_NUM_MSGS, due to bug in reads
}
