...
template <
    ...
    StochRndType stoch_rnd_mode = StochRndType::None /*not used*/>
inline void llk_unpack_reduce_hw_configure(
    ...
    _llk_unpack_reduce_hw_configure_<type, dim>(unpack_src_format[unpA_operand_id], unpack_dst_format[unpA_operand_id]);
}
...
inline void llk_unpack_reduce_init(const std::uint32_t within_face_16x16_transpose = 0) {
    ...
    config.f.out_data_format = (((uint)unpack_dst_format[unpA_operand_id] >> 2) & 0x1) ? (uint)DataFormat::Float16_b
                                                                                       : (uint)DataFormat::Float16;
