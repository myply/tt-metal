...
void kernel_main() {
    ...
    constexpr uint32_t input0_stride = tile_size * input0_num_tiles_width;
    constexpr uint32_t input1_stride = tile_size * input1_num_tiles_width;
    ...
    for (uint32_t i = 0; i < input0_num_tiles_height; i++) {
        ...
        l1_write_addr += input1_stride;  // advance to next row by passing input1 tiles
    }
    ...
    for (uint32_t i = 0; i < input1_num_tiles_height; i++) {
        ...
        l1_write_addr += input0_stride;  // advance to next row by passing input0 tiles
    }
