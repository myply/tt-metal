...
void MultiheadLatentAttentionDecode2::validate(
...
// MODIFIED(QXF)
std::vector<TensorSpec> MultiheadLatentAttentionDecode2::compute_output_specs(
...
operation::ProgramWithCallbacks MultiheadLatentAttentionDecode2::create_program(
    const std::vector<Tensor>& input_tensors,
    ...
    return detail::mla_decode2_multi_core(
        input_tensor_q,
        input_tensor_k,
        input_tensor_v,
        cur_pos_tensor,
...
operation::Hash MultiheadLatentAttentionDecode2::compute_program_hash(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    bool has_cur_pos = optional_input_tensors.at(0).has_value();
    bool has_attn_mask = optional_input_tensors.at(2).has_value();
    return operation::hash_operation<MultiheadLatentAttentionDecode2>(
        this->scale,
