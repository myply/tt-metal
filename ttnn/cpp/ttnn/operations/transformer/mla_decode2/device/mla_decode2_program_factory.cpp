...
#include "mla_decode2_program_factory.hpp"

#include <optional>

#include <tt-metalium/buffer.hpp>
#include "mla_decode2_op.hpp"
...
operation::ProgramWithCallbacks mla_decode2_multi_core(
    ...
    */
    ...
    const auto v_shape = input_tensor_v.get_padded_shape();
    // Use k_shape for S and DH since Q might be different for decode
    uint32_t B = q_shape[1], PNH = q_shape[2], S = k_shape[2], DH = k_shape[3],DC=v_shape[3];
    ...
    uint32_t v_tiles = Sk_chunk_t * DCt * 2;  // double buffer
    uint32_t qk_tiles = PNHt * Sk_chunk_t;
    uint32_t out_im_tiles = PNHt * DCt;
    uint32_t out0_t = PNHt * DCt;
    ...
    const uint32_t out_out_subblock_w = std::min(DCt, dst_size);
    const uint32_t out_out_subblock_h =
        (out_out_subblock_w == DCt) ? (std::min(PNHt, dst_size / out_out_subblock_w)) : 1;

    const uint32_t out_in0_num_subblocks = PNHt / out_out_subblock_h;
    const uint32_t out_in1_num_subblocks = DCt / out_out_subblock_w;
    ...
    // MODIFIED(QXF)
    auto c_in2_config =
        CircularBufferConfig(v_tiles * k_tile_size, {{CBIndex::c_2, k_df}}).set_page_size(CBIndex::c_2, k_tile_size);
    ...
    // MODIFIED(QXF) for TREE REDUCER use
    // cb_out_in
    auto c_out10_config = CircularBufferConfig(out0_t * stats_tile_size, {{CBIndex::c_10, stats_df}})
                             .set_page_size(CBIndex::c_10, stats_tile_size);
    auto cb_out10_id = CreateCircularBuffer(program, core_grid, c_out10_config);
    ...
    // MODIFIED(QXF) 
    std::vector<uint32_t> new_reduce_core_physical_xs;
    std::vector<uint32_t> new_reduce_core_physical_ys;
    new_reduce_core_physical_xs.reserve(num_active_cores/2);
    new_reduce_core_physical_ys.reserve(num_active_cores/2);
    ...
    for (uint32_t i = 0; i < num_active_cores; ++i) {
        ...
        if (do_reduce) {
            ...
            std::cout<<"reduce x: "<<reduce_core_physical.x<<" y: "<<reduce_core_physical.y<<std::endl;
        }
        else{
            reduce_core_noc_x = core.x;
            reduce_core_noc_y = core.y;
            ...
            CoreCoord reduce_core = {(std::size_t)reduce_core_noc_x, (std::size_t)reduce_core_noc_y};
            auto reduce_core_physical = device->worker_core_from_logical_core(reduce_core);
            std::cout<<"worker x: "<<reduce_core_physical.x<<" y: "<<reduce_core_physical.y<<std::endl;
        }
        //MODIFIED(QXF)
        ...
        if(i%2==0){
            ...
            CoreCoord reduce_core = {(std::size_t)reduce_core_noc_x, (std::size_t)reduce_core_noc_y};
            auto reduce_core_physical = device->worker_core_from_logical_core(reduce_core);
            uint32_t reduce_times=0;
            for(uint32_t num_cores_on_right=num_cores_per_head-i%num_cores_per_head;num_cores_on_right>1&&num_cores_on_right%2==0;num_cores_on_right/=2){
                reduce_times++;
            }
            new_reduce_core_physical_xs.push_back((uint32_t)reduce_core_physical.x);
            new_reduce_core_physical_ys.push_back((uint32_t)reduce_core_physical.y);
            std::cout<<"new reducer, reduce_times: "<<reduce_times<<std::endl<<std::endl;
        }
        else{
            std::cout<<"new worker, reduce_times: 0 "<<std::endl<<std::endl;
        }
    }
    std::cout<<"num_cores_per_head: "<<num_cores_per_head<<std::endl<<std::endl;
    ...
    // MODIFED(QXF) ceate multi-Semphore for tree reduction
    std::cout<<"reducer_semaphore_id: "<<reducer_semaphore_id<<std::endl<<std::endl;
    std::cout<<"output_semaphore_id: "<<output_semaphore_id<<std::endl<<std::endl;
    for (uint32_t t=num_cores_per_head;t>1;t=t/2){
        auto reducer_semaphore_id_tree = tt_metal::CreateSemaphore(program, core_grid, 0);
        std::cout<<"tree_reducer_semaphore_id_tree: "<<reducer_semaphore_id_tree<<std::endl<<std::endl;
    }
    ...
    auto compute_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/transformer/mla_decode2/device/kernels/compute/mla_decode2.cpp",
        core_grid,
    ...
    auto reader_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/transformer/mla_decode2/device/kernels/dataflow/reader_mla_decode2_all.cpp",
        core_grid,
    ...
    auto writer_kernels_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/transformer/mla_decode2/device/kernels/dataflow/writer_mla_decode2_all.cpp",
        core_grid,
    ...
    for (uint32_t i = 0; i < num_active_cores; ++i) {
        ...
        writer_rt_args.insert(writer_rt_args.end(), new_reduce_core_physical_xs.begin(), new_reduce_core_physical_xs.end());
        writer_rt_args.insert(writer_rt_args.end(), new_reduce_core_physical_ys.begin(), new_reduce_core_physical_ys.end());
        // compute runtime args
        std::vector<uint32_t> compute_rt_args = {
            do_reduce, do_output, cur_head, cur_batch, core_num_in_reduce, core_num_in_output, cur_pos,reduce_times};
        ...
        [num_active_cores,
         ...
         is_causal](
            ...
            const auto cur_pos_ids =
                static_cast<const ttnn::operations::transformer::MultiheadLatentAttentionDecode2*>(operation)->cur_pos;
