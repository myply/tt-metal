...
void MAIN {
    ...
    // MODIFIED(QXF)
    constexpr uint32_t DCt = get_compile_time_arg_val(4);
    ...
    constexpr uint32_t out_chunk_tiles = Sq_chunk_t * DCt;
    ...
    //MATH(DPRINT << " PSt " <<PSt << " k_num_chunks " <<k_num_chunks << " k_chunk_start " <<k_chunk_start << " k_chunk_end " <<k_chunk_end << ENDL();)
    ...
    for (uint32_t cur_head_work = 0; cur_head_work < num_heads_per_core; ++cur_head_work) {
        mla2_loop<
            ...
            cb_out_o,
            cb_out_m,
            cb_out_l>(k_chunk_start, k_chunk_end, reduce_times, apply_mask_at_last_chunk);

        // do reduction across intermediates from other cores if this is the reduction core
        uint32_t rest_reduce_time = reduce_times;
        MATH(DPRINT << " reduce_times " <<reduce_times << ENDL();)
        if (reduce_times) {
            DeviceZoneScopedN("MLA-REDUCE");
            ...
            if (k_chunk_end - k_chunk_start < k_num_chunks) {
                ...
                for (uint32_t i = 0; i < reduce_times;i++) {
                    MATH(DPRINT << " reduce_time: " <<i << ENDL();)
                // for (uint32_t i = 0; i < num_cores_to_wait; i++) {
                    ...
                    // MODIFIED(QXF)
                    DeviceZoneScopedN("MLA-REDUCE-OTHERS");
                    copy_block(cb_out_in, cb_out_accumulate_im_2, out_chunk_tiles);
                    ...
                    mul_block_bcast_cols_inplace(cb_out_accumulate_im_2, cb_exp_max_diff_2, Sq_chunk_t, DCt);
                    ...
                    MATH(DPRINT << " reduce_time done " << ENDL();)
                }
            }
            // MODIFIED(QXF) Reduce Tree
            // if do reduce, means it should be same with old reduce mothed
            if(do_reduce){
                MATH(DPRINT << " do_reduce: " << ENDL();)
                ...
                pack_reconfig_data_format(cb_out_accumulate_im);
                mul_block_bcast_cols_inplace(cb_out_accumulate_im, cb_cur_sum, Sq_chunk_t, DCt);
            ...
            // so the 3 code lines are same as the last 3 code lines in mla1_loop
                ...
                MATH(DPRINT << " reducer then worker: " << ENDL();)
                copy_block(cb_out_accumulate_im, cb_out_o, out_chunk_tiles);
                copy_block(cb_prev_max, cb_out_m, Sq_chunk_t);
                copy_block(cb_prev_sum, cb_out_l, Sq_chunk_t);
            }
        }
        MATH(DPRINT << " done: " << ENDL();)
    }
    cb_pop_front(cb_q_in, q_chunk_tiles);
    MATH(DPRINT << " compute done: " << ENDL();)
}
}  // namespace NAMESPACE
