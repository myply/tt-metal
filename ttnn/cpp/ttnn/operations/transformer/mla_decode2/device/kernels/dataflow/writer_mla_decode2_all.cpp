...
#include "cpp/ttnn/operations/transformer/mla_decode2/device/kernels/rt_args_common.hpp"
...
void kernel_main() {
    ...
    // MODIFIED(QXF)
    constexpr uint32_t DCt = get_compile_time_arg_val(5);
    ...
    const uint32_t reduce_times = get_arg_val<uint32_t>(arg_idx++);
    // idle core
    if (out_addr == 0) {
        return;
    }
    ...
    arg_idx += num_output_cores;
    tt_l1_ptr uint32_t* new_all_reducer_noc_x = (tt_l1_ptr uint32_t*)(get_arg_addr(arg_idx));
    arg_idx += num_cores/2;
    tt_l1_ptr uint32_t* new_all_reducer_noc_y = (tt_l1_ptr uint32_t*)(get_arg_addr(arg_idx));
    ...
    constexpr uint32_t out_chunk_tiles = PNHt * DCt;
    ...
    // MODIFIED(QXF) TREE REDUCER
    constexpr uint32_t cb_out_in = tt::CBIndex::c_10;
    ...
    //     // DPRINT << "write0 ok: " << ENDL();
    //     return;
    // }
    
    if (reduce_times==0) {
        uint32_t new_reduce_core_index = ((cur_batch * num_cores_per_batch) / num_cores_per_head + cur_head_group)*num_cores_per_head/2
        +core_num_in_reduce/2;
        uint32_t new_reduce_core_noc_x = new_all_reducer_noc_x[new_reduce_core_index];
        uint32_t new_reduce_core_noc_y = new_all_reducer_noc_y[new_reduce_core_index];
        DPRINT << "test2  " <<core_num_in_reduce/2<<ENDL();
        const uint64_t new_in0_sender_semaphore_noc_addr =
        get_noc_addr(new_reduce_core_noc_x, new_reduce_core_noc_y, reducer_semaphore_addr);
        ASSERT(num_heads_per_core == 1);  // if there are workers, then head must be split across workers so there
                                          // should not be more than one head per core
        DPRINT << "worker write to  " <<new_reduce_core_noc_x<<","<<new_reduce_core_noc_y<< ENDL();
        uint32_t new_worker_id_for_reduce=0;
        worker_compute2<out_chunk_tiles, cb_out_worker, cb_out_m, cb_out_l, cb_intermed_out, PNHt>(
            new_in0_sender_semaphore_noc_addr, new_worker_id_for_reduce, new_reduce_core_noc_x, new_reduce_core_noc_y);
        DPRINT << "only worker " << ENDL();
        return;
    }
    ...
    for (uint32_t cur_head = cur_head_group * num_heads_per_core;
        ...
        if (k_chunk_end - k_chunk_start < k_num_chunks) {
            ...
            // MODIFIED(QXF) Reduce Tree
            for (uint32_t block = 0; block < reduce_times; ++block) {
                // MODIFIED(QXF) Reduce Tree
                DPRINT << "reduce_time " <<block<< ENDL();


                // MODIFIED(QXF) use multi-semphore for tree reduction
                // noc_semaphore_wait_min(in0_receiver_semaphore_addr_ptr, block+1);
                uint32_t tree_reducer_semaphore_addr = get_semaphore(get_compile_time_arg_val(10)+block); 
                volatile tt_l1_ptr uint32_t* in0_receiver_semaphore_addr_ptr =
                    reinterpret_cast<volatile tt_l1_ptr uint32_t*>(tree_reducer_semaphore_addr);
                noc_semaphore_wait(in0_receiver_semaphore_addr_ptr, 1);

                DPRINT << "noc_semaphore_wait done" << ENDL();
                //DeviceZoneScopedN("WRITE-REDUCER");
                cb_reserve_back(cb_out_in, out_chunk_tiles);
                DPRINT << "cb_out_in done" << ENDL();
                cb_reserve_back(cb_m_in, PNHt);
                DPRINT << "cb_m_in done" << ENDL();
                cb_reserve_back(cb_l_in, PNHt);
                DPRINT << "cb_l_in done" << ENDL();
                DeviceZoneScopedN("WRITE-REDUCER");
                uint32_t q_write_ptr = get_read_ptr(cb_out_in);
                ...
                // MODIFIED(QXF) Reduce Tree
                //noc_semaphore_set(in0_receiver_semaphore_addr_ptr, 0);
                DPRINT << "reduce_time done" << ENDL();
            }
        }
        // MODIFIED(QXF) Reduce Tree
        // not do_reduce means this core finish collecting other core's result, and plan to send result to another
        // so the 3 code lines are same as the last 3 code lines in mla1_loop
        if (reduce_times!=0 && is_worker==1) {
            uint32_t new_reduce_core_index = ((cur_batch * num_cores_per_batch) / num_cores_per_head + cur_head_group)*num_cores_per_head/2
            +core_num_in_reduce/2/(2*reduce_times)*(2*reduce_times);
            uint32_t new_reduce_core_noc_x = new_all_reducer_noc_x[new_reduce_core_index];
            uint32_t new_reduce_core_noc_y = new_all_reducer_noc_y[new_reduce_core_index];
            ...
            // const uint64_t new_in0_sender_semaphore_noc_addr =
            // get_noc_addr(new_reduce_core_noc_x, new_reduce_core_noc_y, reducer_semaphore_addr);
            uint32_t tree_reducer_semaphore_addr = get_semaphore(get_compile_time_arg_val(10)+reduce_times);
            const uint64_t new_in0_sender_semaphore_noc_addr =
            get_noc_addr(new_reduce_core_noc_x, new_reduce_core_noc_y, tree_reducer_semaphore_addr);

            uint32_t new_worker_id_for_reduce=reduce_times;

            DPRINT << "test1  " <<((cur_batch * num_cores_per_batch) / num_cores_per_head + cur_head_group)*num_cores_per_head/2<< ENDL();
            DPRINT <<" core_num_in_reduce "<<core_num_in_reduce<< "test2  " <<core_num_in_reduce/2/(2*reduce_times)*(2*reduce_times);
            DPRINT << "worker write to  " <<new_reduce_core_noc_x<<","<<new_reduce_core_noc_y<< ENDL();
            worker_compute2<out_chunk_tiles, cb_out_worker, cb_out_m, cb_out_l, cb_intermed_out, PNHt>(
            new_in0_sender_semaphore_noc_addr, new_worker_id_for_reduce, new_reduce_core_noc_x, new_reduce_core_noc_y);
            DPRINT << "reduce and worker done" << ENDL();
            // DPRINT << "write0 ok: " << ENDL();
            return;
        }
        DPRINT << "reduce done" << ENDL();
        ...
        if constexpr (num_kv_heads > 1) {
            ...
            DeviceZoneScopedN("WRITE-MEMORY");
