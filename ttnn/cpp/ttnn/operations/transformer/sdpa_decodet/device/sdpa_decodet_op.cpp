...
void ScaledDotProductAttentionDecodet::validate(
...
std::vector<TensorSpec> ScaledDotProductAttentionDecodet::compute_output_specs(
...
operation::ProgramWithCallbacks ScaledDotProductAttentionDecodet::create_program(
    const std::vector<Tensor>& input_tensors,
    ...
    return detail::sdpa_decodet_multi_core(
        input_tensor_q,
        input_tensor_k,
        input_tensor_v,
        cur_pos_tensor,
...
operation::Hash ScaledDotProductAttentionDecodet::compute_program_hash(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    bool has_cur_pos = optional_input_tensors.at(0).has_value();
    bool has_attn_mask = optional_input_tensors.at(2).has_value();
    return operation::hash_operation<ScaledDotProductAttentionDecodet>(
        this->scale,
