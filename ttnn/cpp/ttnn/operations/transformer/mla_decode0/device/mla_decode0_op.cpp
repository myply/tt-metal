...
void MultiheadLatentAttentionDecode0::validate(
...
// MODIFIED(QXF)
std::vector<TensorSpec> MultiheadLatentAttentionDecode0::compute_output_specs(
    const std::vector<Tensor>& input_tensors) const {

    const auto& input_tensor_q = input_tensors.at(0);
    const auto& input_tensor_v = input_tensors.at(2);
    const ttnn::Shape input_shape_q = input_tensor_q.get_logical_shape();
    const ttnn::Shape input_shape_v = input_tensor_v.get_logical_shape();
    ttnn::Shape output_shape = input_shape_q;
    output_shape[-1] = input_shape_v[-1];
    return {TensorSpec(
        output_shape, TensorLayout(input_tensor_q.get_dtype(), PageConfig(Layout::TILE), output_mem_config))};
}
// std::vector<TensorSpec> MultiheadLatentAttentionDecode0::compute_output_specs(
//     const std::vector<Tensor>& input_tensors) const {
...
operation::ProgramWithCallbacks MultiheadLatentAttentionDecode0::create_program(
    const std::vector<Tensor>& input_tensors,
    ...
    return detail::mla_decode0_multi_core(
        input_tensor_q,
        input_tensor_k,
        input_tensor_v,
        cur_pos_tensor,
...
operation::Hash MultiheadLatentAttentionDecode0::compute_program_hash(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    bool has_cur_pos = optional_input_tensors.at(0).has_value();
    bool has_attn_mask = optional_input_tensors.at(2).has_value();
    return operation::hash_operation<MultiheadLatentAttentionDecode0>(
        this->scale,
