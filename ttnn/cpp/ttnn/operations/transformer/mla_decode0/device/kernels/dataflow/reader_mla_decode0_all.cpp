...
#include "cpp/ttnn/operations/transformer/mla_decode0/device/kernels/rt_args_common.hpp"
...
void kernel_main() {
    ...
    */
    ...
    // MODIFIED(QXF) 
    constexpr uint32_t  v_chunk_tiles= Sk_chunk_t * DCt;
    ...
    for (uint32_t cur_head = cur_head_group * num_heads_per_core;
        ...
        if constexpr (is_paged_attention) {
            for (uint32_t k_chunk = k_chunk_start; k_chunk < k_chunk_end; ++k_chunk) {
                ...
                for (uint32_t row = 0; row < Sk_chunk_t; ++row) {
                    ...
                    uint32_t physical_v_tile_id =
                        virtual_seq_tile_id_to_physical_tile_id<num_kv_heads, block_size_t, DCt>(
                            virtual_v_tile_row_num, cur_head, page_table_ptr);
                    for (uint32_t col = 0; col < DCt; ++col) {
            ...
            const uint32_t v_batch_offset = (cur_batch % Bkv) * num_kv_heads * St * DCt;
            const uint32_t k_head_offset = cur_head * St * DHt;
            const uint32_t v_head_offset = cur_head * St * DCt;

            // Then, read K, V, Mask k_chunk_tiles at a time
            const uint32_t k_chunk_offset = k_chunk_start * Sk_chunk_t * DHt;
            const uint32_t v_chunk_offset = k_chunk_start * Sk_chunk_t * DCt;
