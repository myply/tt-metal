...
void MultiheadLatentAttentionDecode1::validate(
...
// MODIFIED(QXF)
std::vector<TensorSpec> MultiheadLatentAttentionDecode1::compute_output_specs(
...
operation::ProgramWithCallbacks MultiheadLatentAttentionDecode1::create_program(
    const std::vector<Tensor>& input_tensors,
    ...
    return detail::mla_decode1_multi_core(
        input_tensor_q,
        input_tensor_k,
        input_tensor_v,
        cur_pos_tensor,
...
operation::Hash MultiheadLatentAttentionDecode1::compute_program_hash(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors) const {
    bool has_cur_pos = optional_input_tensors.at(0).has_value();
    bool has_attn_mask = optional_input_tensors.at(2).has_value();
    return operation::hash_operation<MultiheadLatentAttentionDecode1>(
        this->scale,
