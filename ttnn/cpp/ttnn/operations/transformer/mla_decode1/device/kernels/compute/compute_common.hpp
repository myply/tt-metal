...
//     cb_pop_front(in_cb, num_tiles);
// }
void transpose_block(uint32_t in_cb,uint32_t out_cb,uint32_t in_row_tiles, uint32_t in_col_tiles,uint32_t out_row_tiles, uint32_t out_col_tiles) {
    copy_tile_to_dst_init_short(in_cb);
    uint32_t in_num_tiles=in_row_tiles*in_col_tiles;
    uint32_t out_num_tiles=out_row_tiles*out_col_tiles;
    cb_wait_front(in_cb, in_num_tiles);
    cb_reserve_back(out_cb, out_num_tiles);
    uint32_t in_tile_idx = 0;
#pragma GCC unroll 0
    for (uint32_t i = 0; i < in_col_tiles; i++) {
        for(uint32_t j=0;j<in_row_tiles;j++){
            if(i<out_row_tiles && j<out_col_tiles){
                acquire_dst();
                copy_tile(in_cb, in_tile_idx, 0 /*dst*/);
                pack_tile(0, out_cb);
                release_dst();
            }
            in_tile_idx += in_col_tiles;
        }
        in_tile_idx = in_tile_idx - in_num_tiles + 1;
    }
...
ALWI void cb_matmul_blocks_with_produced_src(
    const uint32_t& in0_cb,
    const uint32_t& in1_cb,
    const uint32_t& out_cb,
    const uint32_t& M,
    ...
    // MODIFIED(QXF) we try to reuse the k data 
    //cb_pop_front(in1_cb, K * N);
}
/******************************************************************************
...
void mla0_loop(
    ...
    for (uint32_t k_chunk = k_chunk_start; k_chunk < k_chunk_end; ++k_chunk) {
        ...
        if (k_chunk == k_chunk_start) {
            ...
            reconfig_data_format(cb_out_accumulate_im, cb_exp_max_diff);  // DEBUG
            pack_reconfig_data_format(cb_out_accumulate_im);
            mul_block_bcast_cols_inplace(cb_out_accumulate_im, cb_exp_max_diff, Sq_chunk_t, DCt);
...
void mla1_loop(
    ...
    //MATH(DPRINT << "math ok-2: " << ENDL();)
    for (uint32_t k_chunk = k_chunk_start; k_chunk < k_chunk_end; ++k_chunk) {
        // DeviceZoneScopedN("COMPUTE-CHUNK");
        /* QK = Q_CHUNK @ K_CHUNK */
        //MATH(DPRINT << "math ok-1: " << ENDL();)
        reconfig_data_format(cb_q_in, cb_k_in);  // DEBUG
        pack_reconfig_data_format(cb_qk_im);
        ...
        // MODIFIED(QXF)
        mm_block_init_short(
            cb_q_in, cb_k_in, true /*transpose*/, qk_subblock_w /*ct_dim*/, qk_subblock_h /*rt_dim*/, qk_in0_block_w /*kt_dim*/);
        reconfig_data_format(cb_k_in, cb_q_in);
        cb_wait_front(cb_k_in, Sk_chunk_t * DHt);
        DeviceZoneScopedN("COMPUTE-CHUNK");
        cb_matmul_blocks_with_produced_src(
            cb_q_in,
            cb_k_in,
            cb_qk_im,
            Sq_chunk_t,
            ...
            qk_subblock_w,
            true /*transpose*/);
        // MODIFIED(QXF) tranpose k to v
        reconfig_data_format_srca(cb_k_in);
        pack_reconfig_data_format(cb_v_in);
        //copy_block(cb_k_in,cb_v_in,DHt*Sk_chunk_t);
        //transpose_block(cb_k_in, cb_v_in,Sk_chunk_t,DHt);
        //transpose_block(cb_k_in, cb_v_in,DHt,Sk_chunk_t);
        transpose_block(cb_k_in, cb_v_in,DHt,Sk_chunk_t,Sk_chunk_t,DCt);
        pack_reconfig_data_format(cb_qk_im);
        /* QK *= SCALE */
        mul_block_bcast_scalar_inplace(cb_qk_im, cb_scale_in, qk_chunk_tiles);
        ...
        /* OUT_IM = QK @ V_CHUNK */
        //MODIFIED(QXF) no need to reconfig cb_v_in in mla
        reconfig_data_format(cb_qk_im, cb_v_in);  // DEBUG
        pack_reconfig_data_format(cb_out_im);

        // MODIFIED(QXF) what does mm_block_init_short do , this need add a mm_block_init_short?? and will init remove all vals in cb_k_in?? and K is still in transpose because of reuse
        mm_block_init_short(
            cb_qk_im, cb_v_in, false /*transpose*/, out_subblock_w /*ct_dim*/, out_subblock_h /*rt_dim*/, out_in0_block_w /*kt_dim*/);
        // MODIFIED(QXF) after transpose, do we need this cb wait???
        //cb_wait_front(cb_v_in, Sk_chunk_t * DCt);
        ...
        cb_matmul_blocks_with_produced_src(
            ...
            out_subblock_w,
            false /*transpose*/);
        cb_pop_front(cb_v_in, Sk_chunk_t * DCt);
        reconfig_data_format_srca(cb_out_im);
        cb_pop_front(cb_qk_im, qk_chunk_tiles);
