...
typedef struct test_traffic {
    ...
    test_traffic(
        ...
        _get_available_router_cores(num_hops, mcast);
    ...
    void create_kernels(
        ...
        for (auto i = 0; i < num_tx_workers; i++) {
            ...
            log_info(
                LogTest,
                "run traffic_gen_tx at id={}; logical: x={},y={}; physical: x={},y={}",
                tx_device->device_handle->id(),
                core.x,
                core.y,
    ...
    void _get_available_router_cores(uint32_t num_hops, bool mcast) {
        // This is for line mcast
        if (mcast) {
            if (DEFAULT_NUM_HOPS == num_hops) {
                num_hops = rx_devices.size();
            } else if (num_hops != rx_devices.size()) {
                throw std::runtime_error("mcast depth should be equal to number of rx devices");
            }
            if (allow_1st_noc_hop) {
                throw std::runtime_error("1st noc hop not allowed for mcast");
            }
        } else {
        ...
        const auto& end_rx_device = rx_devices[rx_devices.size() - 1];
        for (auto i = 0; i < 16; i++) {
            ...
            try {
                route = tx_device->get_route_to_chip(end_rx_device->mesh_id, end_rx_device->logical_chip_id, i);
            } catch (const std::exception& e) {
                continue;
            }
