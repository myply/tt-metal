 ...
 * | | receiver_semaphore_noc_addr | Encoding of the receiver semaphore location (x,y)+address | uint64_t |
 * DOX-TODO(insert a reference to what constitutes valid coords) | True     | | receiver_data_noc_addr      | Encoding
 * of the receiver source location (x,y)+address    | uint64_t | DOX-TODO(ref to explain valid coords) | True     | |
 * dst_local_l1_addr           | Address in local L1 memory                                | uint32_t | 0..1MB | True |
 * | size                        | Size of data transfer in bytes                            | uint32_t | 0..1MB | True
 * |
 */
template <bool write_barrier = false>
FORCE_INLINE void eth_wait_for_remote_receiver_done_and_get_local_receiver_data(
