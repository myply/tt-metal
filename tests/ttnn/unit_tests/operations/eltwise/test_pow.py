...
def test_binary_sfpu_pow_bug(device, input_shapes):
    torch.manual_seed(0)
    torch_input_tensor_a = torch.randn((32, 64), dtype=torch.bfloat16)
    torch_input_tensor_b = torch.randn((32, 64), dtype=torch.bfloat16)
    torch_output_tensor = torch.pow(torch_input_tensor_a, torch_input_tensor_b)
