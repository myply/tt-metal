...
class TtRobertaModel(nn.Module):
    ...
    def get_extended_attention_mask(self, attention_mask: ttnn.Tensor, input_shape: Tuple[int]) -> ttnn.Tensor:
        ...
        self.ones_const = ttnn.full(extended_attention_mask.padded_shape, 1.0)
        self.mul_const = ttnn.full(extended_attention_mask.padded_shape, self.dtype_min_const)
    ...
    def invert_attention_mask(self, encoder_attention_mask: ttnn.Tensor) -> ttnn.Tensor:
        ...
        if len(encoder_attention_mask.padded_shape) == 3:
            torch_encoder_extended_attention_mask = torch_encoder_attention_mask[:, None, :, :]
        if len(encoder_attention_mask.padded_shape) == 2:
        ...
        self.ones_const = ttnn.full(encoder_extended_attention_mask.padded_shape, 1.0)
        self.mul_const = ttnn.full(encoder_extended_attention_mask.padded_shape, self.dtype_min_const)
    ...
    def forward(
        ...
        past_key_values_length = past_key_values[0][0].padded_shape[2] if past_key_values is not None else 0
