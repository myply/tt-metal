...
class TtMnistModel(torch.nn.Module):
    ...
    def forward(self, x):
        # ttnn.reshape_on_device throws an assertion RuntimeError: TT_ASSERT @ tt_eager/tt_dnn/op_library/reshape/reshape_op.cpp:295: input_tensor_a.padded_shape[3] % TILE_WIDTH == 0 && W % TILE_WIDTH == 0 info:
        # Operand/target width must be a multiple of 32. So using fallback_ops.reshape.
        x = fallback_ops.reshape(x, x.padded_shape[0], 1, 1, 784)
