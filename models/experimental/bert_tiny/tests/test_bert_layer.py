...
def test_bert_layer_inference(
    ...
    pytorch_output_model = hugging_face_reference_model.bert.encoder.layer[encoder_idx]
    ...
    if passing:
        logger.info("Bert_Layer Passed!")
    else:
        logger.warning("Bert_Layer Failed!")

    assert passing, f"Bert_Layer output does not meet PCC requirement {pcc}."
